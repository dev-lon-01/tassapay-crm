...
import { getCredentials, login, searchTransfers } from "@/src/lib/tassapayApi";
import { upsertTransfers, type RawTransfer } from "@/src/lib/transferSync";
import type { ResultSetHeader } from "mysql2";

/**
 * POST /api/sync/transfers
 *
 * Triggers a manual pull-and-upsert of transfer records from the
 ...
 *
 * Response: { fetched, inserted, updated, skipped, syncLogId }
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    const [logRes] = await pool.execute<ResultSetHeader>(
      "INSERT INTO sync_log (started_at, type, status) VALUES (NOW(), 'transfers', 'running')"
    );
    ...
    const raw = await searchTransfers(cookieHeader, loginData, {
      fromdate: fromDate,
      todate: toDate,
    });
    ...
    const result = await upsertTransfers(conn, raw as RawTransfer[]);
    ...
    console.error("[POST /api/sync/transfers]", message);
    if (syncLogId) {
      await pool.execute(
